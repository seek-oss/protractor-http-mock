    ...
    newModule.config(['$provide', '$httpProvider', function($provide, $httpProvider){
        ...
        function getTransformedResponse(response) {
            ...
            for (var i = interceptors.length - 1; i >= 0; i--) {
                var interceptor = getInterceptor(interceptors[i]);
                var isStatusSuccess = !response.status || statusIsSuccessful(response.status);

                if (isStatusSuccess && interceptor.response) {
                    response = interceptor.response(response);
                }

                if(!isStatusSuccess && interceptor.responseError) {
                    response = interceptor.responseError(response);
                }
            }
