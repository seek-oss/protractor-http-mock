...
function mockTemplate() {
    ...
    newModule.config(['$provide', '$httpProvider', function($provide, $httpProvider){
        ...
        function getTransformedRequestConfig(config) {
            // Interceptors can return promises
            ...
            promise = promise.then(function(cfg) {
                if(cfg.transformRequest){
                    cfg.data = transformData(cfg.data, cfg.headers, undefined, cfg.transformRequest);
                }
                return cfg;
            });
        ...
        function transformResponse(response) {
            if (response && response.config && response.config.transformResponse) {
        ...
        function getTransformedResponse(responsePromise) {
        ...
        function wrapWithSuccessError(promise) {
            ...
            if(!myPromise.success || !myPromise.error){
        ...
        function httpMock(config){
            // Apply request interceptors (can return a promise)
            var prom = getTransformedRequestConfig(angular.copy(config));
            prom = prom.then(function(transformedConfig){
                ...
                if(expectation){
                    ...
                    setTimeout(function(){
                        ...
                        resolvedResponse = transformResponse(resolvedResponse);
                    ...
                    return getTransformedResponse(deferred.promise);
                } else {
                    return $http(config);
                }
            });

            return wrapWithSuccessError(prom);
        }
